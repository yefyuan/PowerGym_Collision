...
class Agent(ABC):
    ...
    def compute_action(self, obs: Any, scheduler: EventScheduler):
        ...
        # Set self action first so protocol gating sees a valid action.
        # NOTE: Subordinate coordination depends on `self.action.is_valid()`.
        self.set_action(action)

        # Coordinate subordinate actions if needed (now that `self.action` is set).
        if self._should_send_subordinate_actions():
            self.coordinate(obs, self.action)
